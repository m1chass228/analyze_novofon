...
Прослушай запись и проведи детальный аудит по критериям качества.
...
### КРИТИЧЕСКИЕ ПРАВИЛА ОЦЕНКИ ЗВОНКОВ (ЖЕСТКИЕ ТРЕБОВАНИЯ):

1. ИСКЛЮЧЕНИЯ ИЗ ВОРОНКИ ЗАПИСИ (НЕ ЗАНИЖАТЬ БАЛЛЫ):
Администратор НЕ должен вести клиента к записи, и баллы по критериям "Ведение к записи" (closing_to_appointment) и "Презентация услуг" (presentation) НЕ снижаются (выставляется максимальный балл, либо критерий исключается из оценки, если диалог не предполагал этого), если звонок относится к одной из следующих категорий:
- Уточнения по назначениям: вопросы по дозировкам лекарств, схемам лечения или рекомендациям врача.
- Уточнения по стоимости: клиент узнает цену конкретной услуги, операции или препарата для планирования бюджета.
- Уточнения о готовности результатов: клиент звонит узнать, готовы ли анализы, УЗИ, рентген или гистология.
- Входящие звонки от ЛАБОРАТОРИЙ или контрагентов: передача результатов исследований или рабочие вопросы коллег.
- Запросы о состоянии питомца в стационаре: владелец звонит узнать о самочувствии животного на госпитализации.
При обнаружении такой специфики звонка выставляй максимальный балл за ведение к записи и презентацию, так как цель звонка — исключительно информационная.

2. ОБЪЕКТИВНЫЕ ОТКАЗЫ КЛИЕНТА ("ДОРОГО", "ДАЛЕКО", "НЕ ПОДХОДИТ"):
Если клиент в разговоре открыто озвучивает, что ему:
- "Дорого" (высокая стоимость услуг)
- "Далеко" (неудобно ехать, неподходящий филиал)
- "Не подходит" (нет записи на нужное время, долгий лист ожидания, нет нужного узкого специалиста)
В этих случаях администратор НЕ должен агрессивно настаивать на записи. Если администратор проявил вежливость и эмпатию, балл за "Работа с возражениями" (objections) и "Ведение к записи" (closing_to_appointment) СТРОГО НЕ СНИЖАЕТСЯ. Ставь максимальный балл.

3. ПРАВИЛО ОБРАЩЕНИЯ ПО ИМЕНИ:
- Отсутствие обращения к клиенту по имени НЕ является ошибкой. За критерий "Использование имени" (name_usage) СТРОГО ЗАПРЕЩЕНО снижать общую оценку. 
- Если администратор не узнал имя или не обратился по имени — выставляй за этот критерий МАКСИМАЛЬНЫЙ балл (2 из 2). Если имя было использовано — это отличный бонус, отметь его в комментариях, но штрафовать за его отсутствие нельзя.

### СПЕЦИАЛЬНАЯ МЕТКА "ЗАПРОС АНАЛИЗОВ" (КРИТИЧЕСКИ ВАЖНО):
Если клиент звонит с вопросом о готовности/результатах анализов, УЗИ, рентгена, гистологии и т.п., которые он ранее сдавал/заказывал, И при этом явно следует, что результаты ему ДОЛЖНЫ БЫЛИ отправить (на почту, в WhatsApp, распечатать и т.д.), но НЕ отправили / он их не получил — то поле "dialog_overview" ОБЯЗАТЕЛЬНО должно НАЧИНАТЬСЯ ровно с двух слов "запрос анализов" (без кавычек, без двоеточия), а дальше через тире — обычное описание разговора.
Пример: "запрос анализов - клиент Иванов спрашивал результаты УЗИ, обещали прислать вчера, но так и не отправили."
Эту метку СТРОГО ЗАПРЕЩЕНО ставить в любых других случаях (например если анализы просто ещё не готовы по регламенту и клиенту об этом сообщили, либо если анализ вообще не упоминался).

### СТРУКТУРА ОЦЕНКИ (максимум 43 балла):
1. Установление контакта (max 2)
2. Использование имени (max 2) — ВСЕГДА выставляй 2 балла (не снижать за отсутствие имени).
3. Выяснение потребностей (max 9)
4. Презентация услуг (max 7) — не снижать на информационных звонках / звонках лабораторий.
5. Презентация цен (max 3)
6. Работа с возражениями (max 4) — не снижать при объективных отказах ("дорого", "далеко", "не подходит").
7. Ведение к записи (max 8) — не снижать, если это инфо-звонок, лаборатория, стационар или объективный отказ.
...
{{
  ...
  "total_score": число,
  "category": число,
  "dialog_overview": "Подробный пересказ ключевых моментов. Обязательно укажи здесь, если звонок был информационным (анализы, стационар, дозировки, звонок лаборатории) или зафиксирован обоснованный отказ.",
  "details": {{
